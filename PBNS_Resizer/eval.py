...
for i in range(n_points):
    shapes += [shape_range * np.random.uniform(0, 1, size=(3,))]
...
for s, t in zip(shapes, tightness):
    ...
    writeOBJ(name + 'outfit_{}.obj'.format(step), pred.numpy()[0, ...], model._F)
    writeOBJ(name + 'body_{}.obj'.format(step), B[0, ...], model._smpl['faces'])
    writeOBJ(name + 'complete_{}.obj'.format(step), np.concatenate((B[0, ...], pred.numpy()[0, ...])), np.concatenate((model._smpl['faces'], model._F + len(B[0, :, 0]))))
